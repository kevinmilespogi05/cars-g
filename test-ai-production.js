// Production-ready AI test script
// This version can be used in different deployment environments
...
// Find image files in different environments
function findImageFiles() {
  const environment = process.env.NODE_ENV || 'development';
  const paths = config[environment]?.imagePaths || config.development.imagePaths;
  
  let frontImagePath = null;
  let backImagePath = null;
  
  // Try to find images in different locations
...
async function performRealAIAnalysis(visionClient, frontImagePath, backImagePath) {
  try {
...
async function testRealAI() {
  try {
    ...
    // Find image files
    const { frontImagePath, backImagePath } = findImageFiles();
    
    if (!frontImagePath || !backImagePath) {
      console.error('❌ Image files not found');
      console.log('Tried paths:');
      config[environment]?.imagePaths.forEach(p => console.log(`  - ${p}`));
      console.log('\n💡 Make sure your images are in: public/images/');
      console.log('  - front id.jpg');
      console.log('  - back id.jpg');
      return;
    }
    ...
    // Perform analysis
    const result = await performRealAIAnalysis(visionClient, frontImagePath, backImagePath);
