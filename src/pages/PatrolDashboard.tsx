...
export function PatrolDashboard() {
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      let query = supabase
        .from('reports')
        .select('id, user_id, patrol_user_id, title, description, category, priority, status, location_address, images, created_at, updated_at');
                  ...
                  {selectedReport.patrol_user_id && (
                    <div>
                      ...
                      <div className="flex items-center space-x-1 text-sm text-gray-900">
                        ...
                        <span>Assigned to patrol officer</span>
                      </div>
                    </div>
                  )}
                </div>
