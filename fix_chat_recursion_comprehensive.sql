-- Comprehensive Fix for Chat Participants Infinite Recursion
...
-- ============================================================================
-- 3. CREATE SECURITY DEFINER FUNCTIONS TO BYPASS RLS
-- ============================================================================
...
-- ============================================================================
-- 4. CREATE SAFE POLICIES USING FUNCTIONS
-- ============================================================================
...
-- ============================================================================
-- 5. CREATE OPTIMIZED FUNCTIONS FOR DIRECT MESSAGE ROOMS
-- ============================================================================
...
-- Test is_user_participant_in_room function
SELECT 'is_user_participant_in_room function created successfully' as test_result
WHERE EXISTS (
  SELECT 1 FROM pg_proc 
  WHERE proname = 'is_user_participant_in_room' 
  AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
);

-- Test get_direct_message_rooms_with_participants function
SELECT 'get_direct_message_rooms_with_participants function created successfully' as test_result
WHERE EXISTS (
  SELECT 1 FROM pg_proc 
  WHERE proname = 'get_direct_message_rooms_with_participants' 
  AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
);
...
-- The fix is now complete. The application should work without infinite recursion errors.
-- 
-- Key improvements:
-- 1. All RLS policies now use SECURITY DEFINER functions that bypass RLS
-- 2. No more circular references in policies
-- 3. Optimized functions for direct message room operations
-- 4. Better performance with fewer database queries
--
-- Next steps:
-- 1. Test the chat functionality in your application
-- 2. Update your application code to use the new optimized functions
