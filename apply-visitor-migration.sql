...
-- Run this SQL script in your Supabase SQL Editor or via Supabase CLI

-- Add user_id column to site_visits table to link visitors to authenticated users
ALTER TABLE public.site_visits 
ADD COLUMN IF NOT EXISTS user_id UUID;

-- Add foreign key constraint to profiles table (only if column was just added)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'fk_site_visits_user_id'
    ) THEN
        ALTER TABLE public.site_visits
