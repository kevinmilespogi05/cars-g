-- Fix assigned patroller issue
-- Run this in Supabase SQL Editor

-- First, ensure all ticketing columns exist
...
-- Update existing reports that have patrol_user_id but no assigned_patroller_name
-- Get the username from profiles table
UPDATE public.reports 
SET assigned_patroller_name = COALESCE(
    (SELECT username FROM public.profiles WHERE id = reports.patrol_user_id),
    'Patrol Officer'
)
