...
export const sendVerificationEmail = async (toEmail, otp, type = 'registration') => {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: 'Cars-G <onboarding@resend.dev>',
      to: [toEmail],
      subject: subject,
      html: htmlContent,
    });
    ...
    console.log(`${type} email sent via Resend to:`, toEmail);
    console.log('Message ID:', data.id);
    return true;
    
  } catch (error) {
    console.error(`Error sending ${type} email via Resend:`, error);
    return false;
  }
};
...
export const testEmailConnection = async () => {
  try {
    const { data, error } = await resend.emails.send({
      from: 'Cars-G <onboarding@resend.dev>',
      to: ['202210346@gordoncollege.edu.ph'],
