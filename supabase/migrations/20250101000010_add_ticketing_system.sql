-- Add ticketing system features to reports table
-- This migration adds case numbers, priority levels, assignment tracking, and comments system
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'reports_case_number_key' 
        AND table_name = 'reports'
    ) THEN
        ALTER TABLE public.reports ADD CONSTRAINT reports_case_number_key UNIQUE (case_number);
    END IF;
    
    -- Add check constraint for priority_level if it doesn't exist
    IF NOT EXISTS (
    ...
    -- Add check constraint for assigned_group if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
...
-- Create policies for comments table (with IF NOT EXISTS check)
DO $$ 
BEGIN
    -- Create policy for viewing comments
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE policyname = 'Users can view all comments' 
    ...
    -- Create policy for creating comments
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE policyname = 'Users can update their own comments' 
        AND tablename = 'report_comments'
    ) THEN
