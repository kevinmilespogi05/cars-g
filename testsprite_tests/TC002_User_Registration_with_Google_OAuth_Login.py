...
async def run_test():
    ...
    try:
        ...
        # -> Click the Google OAuth login button to initiate authentication
        frame = context.pages[-1]
        # Click 'Sign in with Google' button to start Google OAuth login
        elem = frame.locator('xpath=html/body/div/div/div/main/div/div[2]/div/div/div[2]/div/form/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the email 'kevin123@gmail.com' into the email field and click Next
        frame = context.pages[-1]
        ...
        # -> Click 'Try again' link to retry Google OAuth login or report issue if retry fails
        frame = context.pages[-1]
        # Click 'Try again' link to retry Google OAuth login
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[2]/c-wiz/main/div[3]/div/div/div/div/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the email 'kevin123@gmail.com' into the email field and click Next to continue Google OAuth login
        frame = context.pages[-1]
        # Input email for Google OAuth login retry
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[2]/c-wiz/main/div[2]/div/div/div/form/span/section/div/div/div/div/div/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('kevin123@gmail.com')
        

        frame = context.pages[-1]
        # Click Next button after entering email for retry
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[2]/c-wiz/main/div[3]/div/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Couldn’t sign you in').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=This browser or app may not be secure.').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Try using a different browser. If you’re already using a supported browser, you can try again to sign in.').first).to_be_visible(timeout=30000)
