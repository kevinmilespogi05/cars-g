...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Input username/email for patrol officer login
        ...
        frame = context.pages[-1]
        # Input password for patrol officer login
        ...
        # -> Check for any new job assignments or notifications related to patrol jobs, then accept a job if available.
        frame = context.pages[-1]
        # Click on 'My Reports' to check assigned jobs or reports relevant to patrol officer
        elem = frame.locator('xpath=html/body/div/div/div/aside/div/div[3]/div[2]/div/div[2]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Close the welcome guide modal and then click on 'My Reports' to view assigned jobs.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click on 'My Reports' to view assigned jobs or reports relevant to patrol officer
        elem = frame.locator('xpath=html/body/div/div/div/aside/div/div[3]/div[2]/div/div[2]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Job Completed Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Patrol officers could not complete the job workflow including accepting jobs, navigation, status updates, and job completion as per the test plan.")
