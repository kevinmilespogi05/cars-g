...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Sign In' to attempt login as a normal user.
        ...
        # -> Input normal user credentials and click Sign In.
        frame = context.pages[-1]
        # Input username/email for normal user login
        ...
        frame = context.pages[-1]
        # Click Sign In button to attempt login as normal user
        elem = frame.locator('xpath=html/body/div/div/div/main/div/div[2]/div/div/div[2]/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Close the welcome guide modal to enable interaction with the reports dashboard filters and export buttons.
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError("Test case failed: The test plan execution failed while validating admin portal access rules and user/report management functions. Access was not properly denied or redirected for normal user, or admin functionalities did not work as expected.")
