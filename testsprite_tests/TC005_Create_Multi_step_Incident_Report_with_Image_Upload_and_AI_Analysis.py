...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Report Issue' link to navigate to the Create Report page
        frame = context.pages[-1]
        # Click on 'Report Issue' link to go to Create Report page
        elem = frame.locator('xpath=html/body/div/div/div/aside/div/div[3]/div[2]/div/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Close the Welcome Guide modal to ensure unobstructed access to the form
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Incident Report Successfully Submitted').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The incident report creation process did not complete successfully, including location selection, image upload with AI danger level detection, anonymous submission, and report syncing as per the test plan.")
